...
public class SearchRoomServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String priceString = request.getParameter("maxprice");
		...
		if(user == null) {
			...
			if (squareMetersString.length() < 1 || priceString.length() < 1 || place.length() < 1) {
				out.println("<!DOCTYPE html>" + "<html>" + "<head><title>Request error</title></head>" + "<body>Er was een invoer fout!</body>" + "</html>");
			} else {
				...
				out.println("<!DOCTYPE html>" + "<html>" + "<head><title>Rooms</title></head>" + "<body>");
